...
parser.add_argument('-ti', '--taxIDin',  type=str, help='TaxID of input organism', required=True)
parser.add_argument('-to', '--taxIDout',  type=str, help='TaxID of output organism', required=True)
...
def load_codon_tables(codon_tables, taxIDin, taxIDout):  #I can probably have the codon table inbuilt
    with open(codon_tables, 'rb') as pickle_file:
        codon_tables = pickle.load(pickle_file)
    return codon_tables[taxIDin], codon_tables[taxIDout]
